    ...
    for tkr in tickerSymbol:
        if tkr != '':
            ...
            if (float(prediction[4]) > (float(last_row['Close']))):
                ...
                # print(output)
                rows.append([str(tkr), str(tickerDf['Close'].iloc[-1]), str(prediction[0]), str(prediction[4])])
                print("**************************************************")
            if (float(prediction[4]) < (float(last_row['Close']))):
                output = ("\nStock " + str(tkr) + "\nPriorCLose:\n" + str(
                    last_row['Close']) + "\n\nPredicition 1 Day: " + str(
                    prediction[0]) + "\n\n: Prediction in 5 Days:" + str(prediction[9]))
                rows.append([str(tkr), str(tickerDf['Close'].iloc[-1]), str(prediction[0]), str(prediction[4])])
                #sendMessage(output)
                # print(output)
    print(rows)
    df = pd.DataFrame(rows, columns=["Top 10 gainers", "Close Value", "Prediction in 1 Day", "Prediction in 5 Days"])
    print(df)
